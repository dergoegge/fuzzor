...
# Try applying the coverage counter reset patch. The patch makes our estimation
# of which harnesses to fuzz for a given code diff more accurate.
git apply ../cov.patch || true

# Restore the depends download
cp -r ../bitcoin_depends_sources ./depends/sources
...
make DEBUG=1 NO_QT=1 NO_BDB=1 NO_ZMQ=1 NO_UPNP=1 NO_NATPMP=1 NO_USDT=1 \
